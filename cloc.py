        ...
        for key, value in data.items():
            if key != "header":
                sql = """insert into code_cloc
                    (createdtime,lang,files,blank,comment,`lines`,label)
                  values ('%s','%s',%d,%d,%d,%d,'%s')""" % (time.strftime('%Y-%m-%d %H:%M:%S'),key,value["nFiles"],value["blank"],value["comment"],value["code"],config.CLOC_LABEL)
                print sql
                c.execute(sql)
        c.close()
        u.conn.commit()
